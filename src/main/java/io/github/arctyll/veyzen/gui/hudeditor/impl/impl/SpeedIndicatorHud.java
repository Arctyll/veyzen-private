...
public class SpeedIndicatorHud extends HudMod {
        ...
        if (Veyzen.INSTANCE.modManager.getMod(getName()).isToggled()) {
            if (isModern()) {
                ...
                Veyzen.INSTANCE.fontHelper.size20.drawString(
                        ...
                        getX() + getW() / 2f - (Veyzen.INSTANCE.fontHelper.size20.getStringWidth((getMeterPerSecond() + " m/s")) / 2f),
                        getY() + 6,
                        getColor()
                );
            } else {
                ...
                Veyzen.INSTANCE.mc.fontRendererObj.drawString(
                        getMeterPerSecond() + " m/s",
                        getX() + getW() / 2 - (Veyzen.INSTANCE.mc.fontRendererObj.getStringWidth(getMeterPerSecond() + " m/s")) / 2,
                        getY() + 6,
                        getColor()
                );
            }
    ...
    private double getMeterPerSecond() {
        double x = Veyzen.INSTANCE.mc.thePlayer.posX - Veyzen.INSTANCE.mc.thePlayer.prevPosX;
        double z = Veyzen.INSTANCE.mc.thePlayer.posZ - Veyzen.INSTANCE.mc.thePlayer.prevPosZ;
