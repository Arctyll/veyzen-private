        ...
        if (Veyzen.INSTANCE.modManager.getMod(getName()).isToggled()) {
            if (isModern()) {
                ...
                Veyzen.INSTANCE.fontHelper.size20.drawString(
                        Veyzen.INSTANCE.mc.isIntegratedServerRunning() ? "Singleplayer" : Veyzen.INSTANCE.mc.getCurrentServerData().serverIP,
                        getX() + getW() / 2f - (Veyzen.INSTANCE.fontHelper.size20.getStringWidth(Veyzen.INSTANCE.mc.isIntegratedServerRunning() ? "Singleplayer" : Veyzen.INSTANCE.mc.getCurrentServerData().serverIP)) / 2f,
                        getY() + 6,
                        getColor()
                );
            } else {
                ...
                Veyzen.INSTANCE.mc.fontRendererObj.drawString(
                        Veyzen.INSTANCE.mc.isIntegratedServerRunning() ? "Singleplayer" : Veyzen.INSTANCE.mc.getCurrentServerData().serverIP,
                        getX() + getW() / 2 - (Veyzen.INSTANCE.mc.fontRendererObj.getStringWidth(Veyzen.INSTANCE.mc.isIntegratedServerRunning() ? "Singleplayer" : Veyzen.INSTANCE.mc.getCurrentServerData().serverIP)) / 2,
                        getY() + 6,
                        getColor()
                );
            }
