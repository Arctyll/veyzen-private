...
public class PingHud extends HudMod {
        ...
        if (Veyzen.INSTANCE.modManager.getMod(getName()).isToggled()) {
            if (isModern()) {
                ...
                Veyzen.INSTANCE.fontHelper.size20.drawString(
                        "Ping: " + getPing(),
                        getX() + getW() / 2f - Veyzen.INSTANCE.fontHelper.size20.getStringWidth("Ping: " + getPing()) / 2f,
                        getY() + 6,
                        getColor()
                );
            } else {
                ...
                Veyzen.INSTANCE.mc.fontRendererObj.drawString(
                        "Ping: " + getPing(),
                        getX() + getW() / 2 - Veyzen.INSTANCE.mc.fontRendererObj.getStringWidth("Ping: " + getPing()) / 2,
                        getY() + 6,
                        getColor()
                );
            }
    ...
    private long getPing() {
        return Veyzen.INSTANCE.mc.getCurrentServerData() == null ? 0 : Veyzen.INSTANCE.mc.getCurrentServerData().pingToServer;
    }
