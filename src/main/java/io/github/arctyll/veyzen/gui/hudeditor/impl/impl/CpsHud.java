...
public class CpsHud extends HudMod {
        ...
        if (Veyzen.INSTANCE.modManager.getMod(getName()).isToggled()) {
            if (isModern()) {
                ...
                Veyzen.INSTANCE.fontHelper.size20.drawString(
                        ...
                        getX() + getW() / 2f - Veyzen.INSTANCE.fontHelper.size20.getStringWidth(isRightClick() ? "CPS: " + getLeftCPS() + " | " + getRightCPS() : "CPS: " + getLeftCPS()) / 2f,
                        getY() + 6,
                        getColor()
                );
            } else {
                ...
                Veyzen.INSTANCE.mc.fontRendererObj.drawString(
                        isRightClick() ? "CPS: " + getLeftCPS() + " | " + getRightCPS() : "CPS: " + getLeftCPS(),
                        getX() + getW() / 2 - Veyzen.INSTANCE.mc.fontRendererObj.getStringWidth(isRightClick() ? "CPS: " + getLeftCPS() + " | " + getRightCPS() : "CPS: " + getLeftCPS()) / 2,
                        getY() + 6,
                        getColor()
                );
            }
    ...
    private boolean isRightClick() {
        return Veyzen.INSTANCE.settingManager.getSettingByModAndName(getName(), "Right click").isCheckToggled();
    }

    private int getLeftCPS() {
        return Veyzen.INSTANCE.cpsHelper.getCPS(0);
    }

    private int getRightCPS() {
        return Veyzen.INSTANCE.cpsHelper.getCPS(1);
    }
}
