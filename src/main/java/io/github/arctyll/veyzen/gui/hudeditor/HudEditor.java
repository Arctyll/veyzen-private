...
public class HudEditor extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Helper2D.drawRectangle(0, 0, scaledWidth, scaledHeight, 0x70000000);

        animateLogo.update();

        GLHelper.startScissor(0, scaledHeight / 2 - 78, scaledWidth, 73);
        Veyzen.INSTANCE.fontHelper.size40.drawString(
            Veyzen.modName,
            scaledWidth / 2f - Veyzen.INSTANCE.fontHelper.size40.getStringWidth(Veyzen.modName) / 2f,
            scaledHeight / 2f + 36 - animateLogo.getValueI(),
            Color.WHITE.getRGB()
        );
        Helper2D.drawPicture(
            scaledWidth / 2 - 25,
            scaledHeight / 2 - 10 - animateLogo.getValueI(),
            50, 50, Color.WHITE.getRGB(), "veyzenlogo.png"
        );
        GLHelper.endScissor();

        animate.update().setReversed(!MathHelper.withinBox(scaledWidth / 2 - 50, scaledHeight / 2 - 6, 100, 20, mouseX, mouseY));
        ...
        Veyzen.INSTANCE.fontHelper.size20.drawString(
            "Open Mods",
            scaledWidth / 2f - Veyzen.INSTANCE.fontHelper.size20.getStringWidth("Open Mods") / 2f,
            scaledHeight / 2f,
            Color.WHITE.getRGB()
        );

        for (HudMod hudMod : hudModList) {
            // Clamp HUD position before rendering
            float modWidthScaled = hudMod.getW() * hudMod.getSize();
            float modHeightScaled = hudMod.getH() * hudMod.getSize();

            if (hudMod.getX() < 0) {
                hudMod.setX(0);
            } else if (hudMod.getX() + modWidthScaled > scaledWidth) {
                hudMod.setX((int) (scaledWidth - modWidthScaled));
            }

            if (hudMod.getY() < 0) {
                hudMod.setY(0);
            } else if (hudMod.getY() + modHeightScaled > scaledHeight) {
                hudMod.setY((int) (scaledHeight - modHeightScaled));
            }
            ...
            if (hudMod.withinMod(mouseX, mouseY)) {
                ...
                if (scroll > 0 && hudMod.getSize() < 2.0f) {
                    hudMod.setSize(Math.min(2.0f, hudMod.getSize() + 0.1f));
                } else if (scroll < 0 && hudMod.getSize() > 0.5f) {
                    hudMod.setSize(Math.max(0.5f, hudMod.getSize() - 0.1f));
                }
            }
            ...
            for (HudMod sHudMod : hudModList) {
                if (
                    ...
                    else if (MathHelper.withinBoundsRange(hudMod.getX() + modWidthScaled, sHudMod.getX() + sHudMod.getW() * sHudMod.getSize(), snapRange))
                        snap.setAll(sHudMod.getX() + sHudMod.getW() * sHudMod.getSize(), sHudMod.getX() + sHudMod.getW() * sHudMod.getSize() - modWidthScaled, false);
                    else if (MathHelper.withinBoundsRange(hudMod.getX() + modWidthScaled, sHudMod.getX(), snapRange))
                        snap.setAll(sHudMod.getX(), sHudMod.getX() - modWidthScaled, false);
                    ...
                    else if (MathHelper.withinBoundsRange(hudMod.getY() + modHeightScaled, sHudMod.getY() + sHudMod.getH() * sHudMod.getSize(), snapRange))
                        snap.setAll(sHudMod.getY() + sHudMod.getH() * sHudMod.getSize(), sHudMod.getY() + sHudMod.getH() * sHudMod.getSize() - modHeightScaled, true);
                    else if (MathHelper.withinBoundsRange(hudMod.getY() + modHeightScaled, sHudMod.getY(), snapRange))
                        snap.setAll(sHudMod.getY(), sHudMod.getY() - modHeightScaled, true);
                    ...
                    if (snap.isSnapping()) {
                        if (!snap.isHorizontal()) {
                            Helper2D.drawRectangle((int) snap.getsPos(), 0, 1, scaledHeight, 0x60ffffff);
                            hudMod.setX((int) snap.getPos());
                        } else {
                            Helper2D.drawRectangle(0, (int) snap.getsPos(), scaledWidth, 1, 0x60ffffff);
        ...
        Helper2D.drawRoundedRectangle(10, scaledHeight - animateSnapping.getValueI(), 40, 40, 4, new Color(25, 103, 255, 230).getRGB(), roundedCorners ? 0 : -1);
        Helper2D.drawPicture(15, scaledHeight + 5 - animateSnapping.getValueI(), 30, 30, Color.WHITE.getRGB(), Style.isSnapping() ? "icon/grid.png" : "icon/nogrid.png");
    }
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (mouseButton == 0) {
            ...
            if (MathHelper.withinBox(scaledWidth / 2 - 50, scaledHeight / 2 - 6, 100, 20, mouseX, mouseY)) {
                mc.displayGuiScreen(new ModMenu());
            }

            if (MathHelper.withinBox(10, scaledHeight - 50, 40, 40, mouseX, mouseY)) {
